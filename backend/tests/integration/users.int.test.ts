...
describe('Users API', () => {
    ...
    it('PUT /api/users/:id atualiza usuário existente', async () => {
        ...
        const res = await request(app)
            ...
            .send({ name: 'Ana Maria', email: 'ana.maria@ex.com' })

        expect(res.status).toBe(200)
        expect(res.body.data).toMatchObject({ name: 'Ana Maria', email: 'ana.maria@ex.com' })
        const updated = await prisma.user.findUnique({ where: { id: user.id } })
        expect(updated).toMatchObject({ name: 'Ana Maria', email: 'ana.maria@ex.com' })
    })
